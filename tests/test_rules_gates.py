...
def test_ui_gate_is_required_on_specification_by_every_route_but_quick() -> None:
    ...
    Since ADR-028 p.1 the gate sits on the design stage, so construction carries
...
def test_unsatisfied_gates_on_standard_construction(
    ...
    """Construction carries the machine gate ``code`` alone (ADR-028 p.1)."""
...
def test_unsatisfied_gates_on_standard_specification(
    ...
    """The design stage requires the specification and the UI gate (ADR-028 p.1)."""
