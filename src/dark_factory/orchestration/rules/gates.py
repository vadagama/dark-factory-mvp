...
ADR-018; the four-route matrix is ADR-023 p.6). Since ADR-029 p.1 the UI gate
...
# design phase since ADR-029 p.1, so it is added to the ``specification`` stage
...
# carries the UI gate (ADR-029 p.1/p.2), review carries the human-confirmed
...
# are redundant since ADR-029 moved ``ui`` into the base set: the union still
