...
class StageContext:
    ...
    ``human_gates`` is the risk-aware human subset of that slice (ADR-028 p.3):
    ...
    human (ADR-028; found in T-043 increment 1, where the risk-widened ``ui``
...
def build_context(
    *,
    ...
    (ADR-005, ADR-028): ``required_gates`` is the full set of the stage (on the
