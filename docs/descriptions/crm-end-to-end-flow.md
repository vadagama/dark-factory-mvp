...
**На чём основано.** `docs/hld.md`, `docs/adr/ADR-001…ADR-022`, `docs/sdd-native-core.md`, `docs/plan.md`, `docs/descriptions/*`, `AGENTS.md`, `.agents/skills/*`, код `src/dark_factory/`, `console/`, `deploy/`, `charts/`, `packs/`.
...
Этот раздел — честная инвентаризация (состояние на 2026-09-16), чтобы не выдать план за работающую систему.
...
| UI-кит, визуальная спецификация, UI-гейты | не реализовано | в плане (T-071); есть слой-заготовка `@small/ui` в engineering pack, роль-скилл дизайна и слот в ChangeSet |
...
| Engineering pack пилотного продукта | реализовано (MR #52) | `packs/web-app`: blueprint React + FastAPI + PostgreSQL, CI продукта, Helm chart приложения, тестовые конвенции; применение к продуктовому репозиторию — первый прогон пилота |
...
Публикация этого документа на страницу Notion пока не выполнена: в контуре нет коннектора Notion, а сама страница по ссылке требует JavaScript-клиента и через обычное чтение не отдаёт содержимое.
...
**Статус изменений в репозитории:** файл добавлен в репозиторий (PR #51) и актуализируется по состоянию кода через стандартный git-цикл задачи — ветка `docs/…`, коммиты, MR против `main` (merge — решение человека).
