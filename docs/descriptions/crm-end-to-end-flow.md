...
**На чём основано.** `docs/hld.md`, `docs/adr/ADR-001…ADR-027`, `docs/sdd-native-core.md`, `specs/001-dark-factory-mvp/tasks.md`, `docs/t043-pilot-journal.md`, `docs/descriptions/*`, `AGENTS.md`, `.agents/skills/*`, код `src/dark_factory/`, `console/`, `deploy/`, `charts/`, `packs/`, `.github/workflows/`.
...
|---|---|---|
| `R0` | косметика, документация | короткий маршрут (`quick`), UI-гейт не требуется |
| `R1` | реестр клиентов, отчёты, импорт | автономно до состояния «готово к merge» |
| `R2` | модель изоляции данных, IAM, интеграции с почтой, аудит | только `standard` и строже; обязателен reconciliation-план; человеческие точки контроля `solution`, `ux` (Construction/UI) и `discovery_release`; release-гейт с вердиктом operation |
| `R3`, `R4` | регуляторные и необратимые изменения | только `architecture`/`foundation`; каждый требуемый гейт стадии — человеческое решение |

Правило монотонности: агент может **повысить** класс, обнаружив риск, но понизить — только формальная политика. Класс выводится детерминированно из наблюдаемых фактов (`classify_risk`), эффективный класс — максимум из заявленного, выведенного и пола маршрута ([ADR-023](../adr/ADR-023-risk-classes-and-control-points.md) п.2). С T-080 условие «риск повышен до R2+» работает как гейт: маршрут с полосой `R0–R1` (`quick`) R2+ не допускает, а обязательные человеческие точки контроля стадии должны иметь approval, привязанный к итоговому SHA; иначе — эскалация `risk_raised_to_r2` с перечнем невыполненных обязательств.
...
**Статус реализации (T-062, 2026-09-16; дополнено T-092, 2026-09-17).** Бюджет-координатор реализован как детерминированные чистые политики (`orchestration/budget/`): единый журнал расхода прогона, лимиты run/role, allowance только ужесточает run-бюджет, резервации `reserve → settle`, вердикты `check/reserve/settle`. Исполнитель стадии превращает вердикт в stop + blocker findings (`orchestration/stages/checks.py`), совокупный расход уезжает в `RunUsageSummary.roles`. Часть TD-007 погашена durable-раннером (T-092): снапшот бюджета и Implementation Contract дюрабельны с миграции 0004 (`execution.budget`, `execution.implementation_contract`), реконструкция запуска их восстанавливает. В рабочий путь координатор по-прежнему не подключён: ни один production-вызывающий не передаёт `budget_check`, полный журнал резерваций процесс-локален, а неизвестная стоимость не консервативна (TD-006, TD-007). Детали — `docs/descriptions/budget.md`.
...
**Статус реализации (T-071, 2026-09-16).** Small UIKit существует как отдельный версионированный пак `packs/ui` 0.1.0 (ADR-014): 12 компонентов и 5 паттернов поверх Radix с адаптированным shadcn-слоем, токены DTCG (`tokens/*.tokens.json` → `tokens.css` и `tokens.ts`, drift-тест генерата против источника), Storybook 10 как исполняемая спека (45 историй) и четыре машинных UI-гейта — UIKit-policy ESLint (импорты только из корня `@small/ui`, Radix — только внутри кита), stylelint (цвета только через `var(--small-*)`) плюс token-drift, axe/WCAG 2.2 AA и visual regression против коммиченных бейзлайнов. Гейты исполняет фабричный CI (`uikit-lint`, `uikit-typecheck`, `uikit-test`, `uikit-gates`, `uikit-storybook`, `uikit-visual` в закреплённом контейнере Playwright); эталоны регенерирует отдельный workflow `uikit-baselines.yml` по метке на change request — обновление эталона проходит через ревью, а не через автора кода. Blueprint `packs/web-app` 0.2.0 несёт полную копию кита (`frontend/packages/ui`, паритет проверяет `tests/test_packs_ui.py`). Роль `design` реализована после этой работы (T-081: профиль `DESIGN_PROFILE` и скиллы `ux-flow`, `accessibility-review`; вместе с T-082 каталог ролей ADR-007 полон — все девять ролей). Чего пока нет: UI-гейтов уровня продукта (валидатор покрытия состояний экрана, продуктовый Playwright E2E) и визуальной спецификации конкретного продукта — это фаза дизайна экранов пилота; неавтоматизируемая часть WCAG 2.2 AA подтверждается человеком по checklist `packs/ui/rules.md`. Детали — `docs/descriptions/ui-kit.md`.
...
| `risk_raised_to_r2` | обнаружен риск R2+ и не выполнены обязательства класса: маршрут его не допускает либо обязательные точки контроля не имеют approval на итоговом SHA (T-080) |
...
**Статус реализации (T-092, ADR-024/ADR-025, 2026-09-17).** Разрыв «сквозной сервис не собран» закрыт. Durable-драйвер запуска `factory run advance` исполняет ровно одну стадию за вызов (lease → попытка → решение через `apply_result` → атомарная запись с successor-стадиями и outbox-событием; повтор того же логического действия инертен), а связывание адаптеров живёт в composition root `dark_factory.runtime` — единственном слое, которому разрешён импорт адаптеров. Агентные стадии — спецификация и планирование (продукт: скиллы `spec-authoring`, `change-request`), construction (разработка: `implementation`), review_verification (качество: `code-review`): harness-исполнитель с инструментами роли и изолированным worktree от закреплённой ревизии, публикация ветки, коммита и change request — идемпотентные внешние эффекты; попытка без изменений останавливается `blocked` до создания ветки и CR. Ожидания разрешаются той же попыткой: waiting-чекпоинт вытесняется финалом при наблюдении фактов (`FactsProvider`: pipeline на итоговом SHA, merge-наблюдение CR) — гейт не «passed» без наблюдаемого успеха на итоговом SHA. Bounded rework врезан в драйвер: повтор блокирующих находок — `plan_rework`, лимиты enforced flow'ом. Release — не агентная работа: `ReleaseStageExecutor` продвигает ожидаемый immutable digest в GitOps-репозиторий одним promotion-MR и паркуется до rollout-фактов (Argo sync/health, smoke); без ожидаемого digest — честный `blocked`. Merge — manual mode: merge совершает человек на провайдере, драйвер наблюдает факт через approval, привязанный к head. Ограничения: удаления файлов в дельте MVP не представимы; координатор бюджета в рабочий путь не подключён (TD-006/TD-007); обязательные решения R2+ подаёт вызывающий (TD-014); живой прогон агентных стадий на пилотном репозитории — T-043. Детали — `docs/descriptions/runtime.md`, `docs/descriptions/execution.md`.
...
**Статус реализации (S3, T-092).** Политика врезана в durable-драйвер: человеческое решение наблюдается на провайдере (`MergeRequestPort.observe` — review, привязанный к head); без привязанного approval стадия паркуется в waiting (`manual_merge_required`), повторное применение результата с approval продвигает. Автоматический merge по-прежнему выключен (T-085).
...
| Продвижение по стадиям | durable-драйвер `factory run advance` — по таблице переходов, с обязательными гейтами и бюджетом |
...
Этот раздел — честная инвентаризация (состояние `main` на 2026-09-18: durable-раннер T-092 по ADR-024/ADR-025, полные профили ролей T-081/T-082, CI-тогглы ADR-026/027, инкремент 0 пилота T-043), чтобы не выдать план за работающую систему.
...
| Независимая приёмка и решения о доработке | реализовано, врезано в драйвер (T-092 S3) | классификация находок, лимит раундов; повтор находок — `plan_rework` из рабочего пути |
| Бюджет-координатор и allowance-политики | реализовано как чистые политики | журнал расхода run/role, резервации, вердикты; в рабочий путь не подключён, но снапшот бюджета дюрабелен (миграция 0004) — частичное погашение TD-007 (T-062, T-092) |
| Durable-драйвер запуска и composition root | реализовано (T-092, ADR-024/025, срезы S1–S4) | `factory run advance` — одна стадия за вызов, replay-first; связывание адаптеров — `dark_factory.runtime`; агентные стадии через harness и worktree; release — детерминированный promotion digest'а; живой прогон — за пилотом (T-043) |
| Профили исполняемых ролей | реализовано (T-081, T-082) | каталог ADR-007 полон — девять ролей с профилями и скиллами (design/architect: ux-flow, accessibility-review, impact-analysis, adr-proposal; infrastructure/security/ci-cd/operation: infra-design, threat-model и др.); агентные стадии используют product/develop/quality |
| Параллельные ветви внутри стадии | реализовано, в production не подключено | есть тесты, нет вызовов из рабочего пути |
| Вычисление гейтов в рабочем пути | реализовано (T-092 S3) | ожидания разрешаются той же попыткой через `FactsProvider`: pipeline на итоговом SHA, merge-наблюдение CR, release-факты rollout; живой прогон — за пилотом (T-043) |
| UI-кит, визуальная спецификация, UI-гейты | реализовано (T-071, MR #54) | `packs/ui` 0.1.0: 12 компонентов + 5 паттернов поверх Radix, токены DTCG, Storybook как исполняемая спека, четыре гейта в фабричном CI (UIKit-policy ESLint, stylelint, axe/WCAG 2.2 AA, visual regression); копия кита в blueprint `packs/web-app` (паритет — тестом); исполняемая роль `design` реализована (T-081), UI-гейты уровня продукта и визуальная спецификация пилотного продукта — фаза дизайна пилота |
...
| Риск-матрица R0–R4 как гейт | реализовано (T-080) | класс выводится детерминированно из фактов, у маршрутов есть полосы классов, условие R2+ — машинная проверка обязательств и человеческих точек контроля ([ADR-023](../adr/ADR-023-risk-classes-and-control-points.md)); авто-merge R0/R1 как политика — по-прежнему выключен (T-085) |
| Автоматический merge низкого риска | выключен | включается по накопленной статистике пилота |
| Доставка: bootstrap кластера, Helm, Argo CD, OCI-сборка, smoke | реализовано, живой контур поднят и подтверждён деплоем пилота | bootstrap, Argo CD (app-of-apps) и раннер подняты, gitops-репозиторий создан; пилотный продукт собран CI (9/9 джоб, digest-pinned) и подключён к apps-dev; живой деплой завершён (Argo Synced/Healthy, миграционный хук post-фаз с wait-for-database, smoke `/api/healthz` и `/` зелёные — инкремент 0 пилота; TD-010/TD-029 закрыты); digest-MR на новой версии → Argo → откат revert'ом — за инкрементом 1 (TD-020) |
| Engineering packs продуктового репозитория | реализовано (MR #52, MR #54), применены пилотом | `packs/web-app` 0.2.1: blueprint React + FastAPI + PostgreSQL со Small UIKit, CI продукта, Helm chart приложения (миграции — post-install hook), тестовые конвенции; `packs/ui` 0.1.0: Small UIKit с UI-гейтами; применены к пилотному продукту `dark-factory-product-1` (инкремент 0): дефекты шаблона, невидимые на рендере, поймал живой CI и починены канонично в паке (фабричный `create_app`, apt-upgrade базы под fail-closed trivy, пин `psycopg[binary]<3.3`) |
| Console фабрики | реализовано | шесть экранов — наблюдение, точки вмешательства и управление этапами CI (`/ci`, ADR-027); проверено в кластере |
| Репозитории GitOps и записей прогонов | GitOps создан и подключён; записи прогонов — seed | gitops-репозиторий создан (seed запушен, `main` защищена с отклонениями GitHub Free — TD-026); репозиторий записей прогонов по-прежнему ждёт решения человека |

Известный технический долг, влияющий на сквозной сценарий: внешний приёмник телеметрии не подключён, межузловая передача трассировки через границу CI-задания не реализована, отбор чувствительных атрибутов перед экспортом телеметрии отсутствует (TD-003, TD-004); публикация записей прогонов сделана по P0-минимуму — публикатор раскладывает файлы локально, но не коммитит их в `dark-factory-runs`, single-writer-протокол, retention и подпись манифеста не реализованы (TD-005); бюджет: неизвестная стоимость не консервативна, полный журнал резерваций процесс-локален — снапшот бюджета дюрабелен, остальное открыто (TD-006, TD-007); факты риска наблюдаются частично, а обязательные решения R2+ подаёт вызывающий стадии (TD-013, TD-014); агентная стадия MVP не выражает удаления файлов и двоичный контент; blueprint engineering pack прошёл живой деплой (TD-010 закрывается), но без SAST/SCA- и SBOM-гейтов фабрики и без полного reproducible-build контракта (TD-011, TD-012); первый живой прогон вскрыл слепые зоны валидации — admission (LimitRange против requests чарта, TD-025) и защиту `main` на GitHub Free (TD-026); структурированный ответ LLM идёт prompted-режимом, стоимость вызова не измеряется (TD-015 — продолжение TD-006); пин `psycopg[binary]<3.3` из-за wheel-SBOM psycopg 3.3.x в сканере образов — кандидат в реестр (журнал пилота); TD-022/TD-023/TD-024 (исполнимость агентной стадии, entry point, commit/push) погашены T-092; TD-009 (UIKit-заготовка вместо кита) погашен задачей T-071; запись аудита в отдельный репозиторий требует доверенного CI-контура.
...
Публикация этого документа на страницу Notion не выполнена: автоматической публикации из репозитория нет, ручная требует сессии с коннектором Notion, а сама страница по ссылке требует JavaScript-клиента и через обычное чтение не отдаёт содержимое.
...
2. Каждый блок кода с пометкой `mermaid` вставить отдельным блоком-диаграммой: в Notion это делается командой `/mermaid` и вставкой кода — так отрисуются все девятнадцать диаграмм документа.
...
**Статус изменений в репозитории:** файл добавлен в репозиторий (PR #51), актуализирован после T-062 и T-041 (PR #53), после T-071 и после T-092 с инкрементом 0 пилота T-043 (текущее обновление, 2026-09-18). Дальше обновляется по состоянию кода через стандартный git-цикл задачи — ветка `docs/…`, коммиты, MR против `main` (merge — решение человека).
