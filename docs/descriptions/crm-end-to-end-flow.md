...
| Доставка: bootstrap кластера, Helm, Argo CD, OCI-сборка, smoke | реализовано, живой контур поднят частично | bootstrap, Argo CD (app-of-apps) и раннер подняты, gitops-репозиторий создан; пилотный продукт собран CI (9/9 джоб, digest-pinned) и подключён к apps-dev; деплой дошёл до pull образов (блокер — приватные ghcr-пакеты), smoke после снятия; живой прогон digest-MR → Argo → smoke — за пилотом (TD-010, TD-020) |
