...
**Статус реализации (T-062, 2026-09-16).** Бюджет-координатор реализован как детерминированные чистые политики (`orchestration/budget/`): единый журнал расхода прогона, лимиты run/role, allowance только ужесточает run-бюджет, резервации `reserve → settle`, вердикты `check/reserve/settle`. Исполнитель стадии превращает вердикт в stop + blocker findings (`orchestration/stages/checks.py`), совокупный расход уезжает в `RunUsageSummary.roles`. В рабочий путь пока не подключён: ни один production-вызывающий не передаёт `budget_check`, журнал координатора процесс-локален, а неизвестная стоимость не консервативна (TD-006, TD-007). Детали — `docs/descriptions/budget.md`.
...
Этот раздел — честная инвентаризация (состояние `main` на 2026-09-16, включая T-071), чтобы не выдать план за работающую систему.
...
| Бюджет-координатор и allowance-политики | реализовано как чистые политики | журнал расхода run/role, резервации, вердикты; в рабочий путь исполнителя стадий не подключён (T-062) |
...
| UI-кит, визуальная спецификация, UI-гейты | реализовано (T-071, MR #54) | `packs/ui` 0.1.0: 12 компонентов + 5 паттернов поверх Radix, токены DTCG, Storybook как исполняемая спека, четыре гейта в фабричном CI (UIKit-policy ESLint, stylelint, axe/WCAG 2.2 AA, visual regression); копия кита в blueprint `packs/web-app` 0.2.0; UI-гейты уровня продукта и исполняемая роль `design` — план (T-081) |
...
| Engineering packs продуктового репозитория | реализовано (MR #52, MR #54) | `packs/web-app` 0.2.0: blueprint React + FastAPI + PostgreSQL со Small UIKit, CI продукта, Helm chart приложения, тестовые конвенции; `packs/ui` 0.1.0: Small UIKit с UI-гейтами; применение к продуктовому репозиторию — первый прогон пилота |
...
Известный технический долг, влияющий на сквозной сценарий: внешний приёмник телеметрии не подключён, межузловая передача трассировки через границу CI-задания не реализована, отбор чувствительных атрибутов перед экспортом телеметрии отсутствует (TD-003, TD-004); публикация записей прогонов сделана по P0-минимуму — публикатор раскладывает файлы локально, но не коммитит их в `dark-factory-runs`, single-writer-протокол, retention и подпись манифеста не реализованы (TD-005); бюджет: неизвестная стоимость не консервативна, журнал координатора процесс-локален (TD-006, TD-007); blueprint engineering pack валидирован структурно (helm lint/template), живой деплой в apps-dev — за первым сквозным прогоном (TD-010); продуктовый CI пака собирает образы, но не имеет SAST/SCA- и SBOM-гейтов фабрики, а его Dockerfile'ы не воспроизводят reproducible-build контракт (TD-011, TD-012); TD-009 (UIKit-заготовка вместо кита) погашен задачей T-071; запись аудита в отдельный репозиторий требует доверенного CI-контура.
...
**Статус изменений в репозитории:** файл добавлен в репозиторий (PR #51), актуализирован после T-062 и T-041 (PR #53) и после T-071 (текущее обновление). Дальше обновляется по состоянию кода через стандартный git-цикл задачи — ветка `docs/…`, коммиты, MR против `main` (merge — решение человека).
