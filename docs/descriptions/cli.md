...
| Заведение задачи с брифом, сценарием и лимитом; «следующий шаг» | `change create` / `change status` (T073, ADR-033) |
| Цикл обсуждения фазы: ответ, замечание, «на доработку», согласование | `change answer` / `comment` / `rework` / `approve` (T086, ADR-034) |
| Артефакты изменения в git: дерево, документ, правка, diff, ревизии | `change artifacts list|show|edit|diff|versions` (T086, ADR-035) |
| Решения архитектуры, «Запросить альтернативу», UI-спека изменения | `change decisions` / `alternative` / `ui` (T093/T094, ADR-039) |
| Baseline-паки в репозиторий продукта | `product bootstrap` (T069/M2, ADR-031 п.3) |
...
flowchart TD
    ...
    D --> CHG["change create/status\ncli/changes.py + cli/guidance.py"]
    D --> REC["reconcile\ncli/reconcile.py"]
    ...
    PSTORE --> PG
    CHG --> GSTORE["orchestration/state/guidance.py\nфакты → orchestration/guidance.py\n(та же проекция, что у API)"]
    GSTORE --> PG
...
| `change create`, `change status` | PostgreSQL через `orchestration/state/change_store.py` (`ProductRepository`, `ChangeRepository`, `AuditRepository`) и `orchestration/state/guidance.py` (последний run и его результат — для `Guidance`) |
...
| `change create` | `--product`*, `--title`*, `--limit-usd`*, бриф: `--problem`, `--goal`, `--constraint` (повторяемый), `--out-of-scope` (повторяемый) **или** `--brief-json <path\|->`; `--scenario` (`specs_only`/`full`, default `full`), `--token-limit`, `--risk-class` (default `R1`), `--description`, `--id`, `--json` | Intake задачи для продукта с брифом, сценарием и лимитом (T073/T071); идемпотентно по `--id`; печатает блок «Следующий шаг» | 0/2 |
| `change status` | `--id`*, `--json` | Задача, её последний run и блок «Следующий шаг» (`Guidance`, T074) | 0/2 |
| `change phases` | `--id`*, `--json` | Восемь фаз задачи с состоянием, ревизией фазы, счётчиками и итерацией; текущая помечена `*` (T098, ADR-039) | 0/2 |
| `change decisions` | `--id`*, `--json` | Карточки архитектурных решений (ADR) изменения с производным статусом, открытым поручением и затронутыми артефактами — тот же view, что `GET /changes/{id}/decisions` (T093, ADR-039) | 0/2 |
| `change alternative` | `--id`*, `--decision`*, `--instruction`*, `--comment` (повторяемый), `--json` | «Запросить альтернативу»: поручение на доработку фазы `architecture` по одному решению; в той же транзакции — `rejected` по гейту фазы (T093) | 0/2 |
| `change ui` | `--id`*, `--section` (`scenarios`/`screens`/`links`), `--json` | UI-спека изменения: сценарии, экраны, связи — тот же view, что `GET /changes/{id}/ui` (T094) | 0/2 |
...
**Contract-опции `run advance` (T-016, ADR-018 p.3)** — все необязательны, без них поведение прежнее:

- `--contract-json <path|->` — Implementation Contract запуска: читается (`-` — stdin), валидируется pydantic-схемой и прикрепляется к резолвнутому запуску в той же транзакции, что и решение стадии — вход в construction в этом же advance видит контракт. Идемпотентно и без подмены (`RunStore.attach_contract`): запуск без контракта записывает переданный; идентичный — no-op; другой — `ContractConflictError` → exit 2, ничего не записано (утверждённая граница работающего изменения не подменяется). Битый файл/JSON/схема — exit 2 до обращения к store.
- `--approve-contract` — human-утверждение на загруженном контракте (`approved_by=Role.PRODUCT`, `decided_at=now(UTC)`; решение оператора, ADR-011). Флаг без `--contract-json` — exit 2; контракт, уже несущий approval, вместе с флагом — двусмысленность, exit 2. Утверждённый контракт снимает блокировку входа в construction; неутверждённый прикрепляется и честно даёт `blocked`.
...
С M2 (T080/T081) `run advance` внутри своей транзакции собирает typed-входы обсуждения фазы (`orchestration/state/conversations.py: load_conversation_inputs` — ответы, открытые вопросы и замечания, поручение) и передаёт их в `StageContext.conversation`; наблюдаемые факты гейтов дополняются решениями из store, привязанными к голове CR, и поручениями фазы (`with_store_facts`), так что согласование из API/Console резолвит ожидание так же, как ревью на провайдере, а pending-поручение — в раунд доработки; после продвижения `record_stage_outcome` сохраняет вопросы агента из `StageResult.questions` (детерминированные id — replay инертен), переводит поручение `pending → in_progress → done` со сводкой, отмечает замечания `addressed`, а при `blocked` — `escalated`.
...
**`product bootstrap`** (M2) — операторская команда «Подготовить baseline» поверх `RepositoryProvisioningPort.bootstrap_baseline` (T069): применяет `--pack` (по умолчанию `product-baseline`) одним коммитом в default-ветку репозитория продукта, replay по детерминированному ключу `bootstrap:<id>:<packs>`, audit `product.bootstrap`. Без шва `provisioning` или с адаптером, который bootstrap не умеет (локальное зеркало, `ProvisioningOperationUnsupportedError`), — exit 2, ничего не записано; сбой адаптера — exit 1 с фиксированным текстом. `--json` — `{"product", "result"}` (`BaselineBootstrapResult`: `revision`, `applied_packs`). Это шаг перед первым `run advance` на пустом репозитории: `ScmRevision` иначе не найдёт `main`.

### 3.11. `change create | status` (T073, T071, T074, ADR-033)

Операторская сторона intake (`cli/changes.py`). **`change create`** заводит задачу для зарегистрированного продукта: репозиторий берётся из реестра (`ProductRepository`), неизвестный `--product` — exit 2 до записи. Бриф — из флагов (`--problem`, `--goal`, повторяемые `--constraint`/`--out-of-scope`; автор `operator`) или из JSON (`--brief-json`, путь или `-` для stdin; та же форма, что ответ `POST /briefs/formulate`, поэтому бриф, сформулированный агентом, передаётся как есть, с `formulated_by=agent` и `source_text`); совмещать оба способа нельзя (exit 2). Бриф без проблемы или цели сохраняется как `draft` — задача создаётся, а «следующий шаг» говорит, что дополнить. Лимит `--limit-usd` парсится командой (не число, ≤ 0, больше 4 знаков — exit 2 с текстом, не трейс argparse); `--token-limit` — второе измерение. `--id` — клиентский id (по умолчанию генерируется `chg_<12 hex>`); повтор с тем же `--id` — replay без записи, audit `change.intake` с `outcome` `created`/`replayed` и `actor=cli`. Текстовый вывод: строка результата, факты задачи (`render_change_text`: продукт, репозиторий, сценарий, лимит, риск, статус брифа и недостающие поля, problem/goal) и блок «Следующий шаг».

**`change status`** печатает задачу, её последний run (`run <id>: <status> (state_revision=…, cost_used=…)` или `run: none`), с M2 — обсуждение изменения (`render_discussion_text`: открытые вопросы с вариантами и якорями, отвеченные-но-не-применённые, открытые замечания, поручения с раундом и сводкой), строку гейта текущей фазы (`render_gate_text`: `gate <phase>: available|closed (revision=…, approved=…, rework=n/m)` плюс причины и устаревшие согласования) и тот же блок «Следующий шаг». `--json` — `{"change", "run", "questions", "comments", "rework_orders", "phase_gate", "guidance"}` (у `create` — `{"outcome", "persisted", "change", "guidance"}`).

### 3.12. `change answer | comment | rework | approve | artifacts` (T086, T087, ADR-034/ADR-035)

Операторская сторона цикла «вопрос → ответ → правка → сводка → согласовать / на доработку» (план ChangeSet §6, M2). Команды исполняют **те же** операции, что API (`orchestration/state/conversation_ops.py` — ответ, замечание, поручение, решение по фазе; `orchestration/artifacts.py` — артефакты), поэтому CLI и Console не могут расходиться (ADR-033 p.3); типизированные отказы операций (`ConversationNotFoundError`/`ConversationConflictError`/`ConversationInputError`) — exit 2 `invalid_input` с текстом причины. Каждая команда завершается блоком «Следующий шаг». Фаза по умолчанию — фаза последнего run, до первого run — `requirements` (`phase_gate.discussion_phase`).

- **`change answer`** — ответ оператора на вопрос агента: проверяется по типу (`choice` — только из вариантов, `number` — число, пустой — отказ), записывается как человеческое решение (`answered`), повтор того же ответа — replay без записи. Audit `question.answer`, `actor=cli`.
- **`change comment`** — замечание к фрагменту `--artifact` (+ `--anchor`: стабильный id `REQ-001`/`AC-2`/slug заголовка), привязанное к голове ветки изменения, когда связан шов `repository`; замечание **не** запускает доработку (ADR-034 p.2). Audit `comment.add`.
- **`change rework`** — явное «На доработку»: поручение из `--comment`/`--question` (повторяемые) и `--instruction` (хотя бы одно), одно на фазу одновременно (второе — exit 2), неизвестный id — exit 2; в той же транзакции пишется version-bound `rejected` по гейту фазы. Сам раунд исполняется следующим `run advance`: pending-поручение резолвит ожидание human-gated стадии в `ReworkAction` (T081), счётчик — `budget.used_rework_rounds`.
- **`change approve`** — согласование фазы на текущей ревизии через прекондиции гейта (T087): закрытый гейт (блокирующие вопросы, ожидающая/идущая доработка, нет ревизии) — exit 2 с причинами; `--waive` с `--comment` — пропуск фазы с основанием (ADR-032 p.5), без основания — exit 2; `--revision` нужна без шва `repository`. `--json` — `{"outcome", "decision", "phase_gate", "guidance"}`. С M3 решение несёт `phase` (`--phase requirements|architecture|interface`, по умолчанию — текущая фаза из проекции ADR-039), привязывается к ревизии артефактов **своей** фазы, а пропуск (`--waive`) допустим без ревизии — у backend-only изменения UI-артефактов нет.
- **`change phases`** — проекция восьми фаз (`orchestration/phases.py`, `GET /changes/{id}/phases`): `F<n> <фаза> <state> rev=<ревизия фазы> q=<вопросов>(!<блокирующих>) c=<замечаний> iter=<раундов>`, текущая фаза помечена `*`; процента готовности нет (ADR-037 p.6).
- **`change artifacts <list|show|edit|diff|versions>`** — дерево `.factory/changes/**` на голове ветки изменения (`list`; «ветки ещё нет» и «артефактов нет» различимы), документ на ревизии (`show`, сырое содержимое в stdout или `--json` с свойствами и якорями), правка (`edit --file <path|-> --base-revision <r>` — один коммит через `RepositoryPort.publish_commit`; конфликт — файл менялся после базы — exit 1 без записи; неизменённое содержимое — no-op; черновик пути удаляется, вопросы к исчезнувшему фрагменту становятся `stale`), `diff --from --to` (unified) и `versions` (коммиты пути). Без шва `repository` — exit 2 до обращения к store (ADR-035 p.1: документ, который фабрика не может прочитать, не показывается).

**Блок «Следующий шаг»** (`cli/guidance.py: render_guidance_text`) — рендер `Guidance`, вычисленного ядром (`orchestration/state/guidance.py` собирает продукт, последний run и `NextAction` его последнего результата; `orchestration/guidance.py` — чистая проекция): `Следующий шаг: <headline>` · `почему:` · `→ <primary> (cli: …) (api: …)` (недоступное действие — `— недоступно: <причина>`) · `также:` · `блокеры:` (`что — снимает: кто — как`) · `после:`. CLI ничего не решает сам — тот же объект отдаёт `GET /changes/{id}/guidance`, и Console рендерит его же (ADR-033 p.3); `tests/integration/test_cli_changes.py` сверяет JSON CLI с проекцией из store побайтно.

### 3.13. `change decisions | alternative | ui` (T093, T094, ADR-039)

Операторская сторона фаз «Архитектура» и «Интерфейс» (M3). Все три команды читают документы из git и требуют шва `repository`: без него — exit 2 `invalid_input` **до** обращения к store (ADR-035 p.1: карточка или экран, которые фабрика не может прочитать, не показываются). Неизвестная задача и недоступный store — exit 2.

- **`change decisions`** — карточки ADR из `design/decisions/**` на голове ветки изменения (`orchestration/state/decisions.py: load_decisions_view` — та же сборка, что у `GET /changes/{id}/decisions`). Текст (`render_decisions_text`): заголовок `decisions of <id>: revision=<ревизия фазы architecture> approved=yes|no`; на карточку — `- <id>: <title>`, `status=<производный> document_status=<из frontmatter> rev=<ревизия ADR>`, `proposal: <первая строка раздела «Решение»>`, `alternatives=<n> impact=<…> pending_alternative=<id поручения или ->`, при наличии — `affected: <пути>` и строки `! <ошибка разбора>`; без ADR — `(no ADR documents on the change branch)`; ошибки view — строки `! …` в конце. Завершается блоком «Следующий шаг». `--json` — `{"decisions": DecisionsView, "guidance": Guidance}`.
- **`change alternative`** — «Запросить альтернативу» по одному решению: `--decision` — `id` из frontmatter ADR (или имя файла без расширения), `--instruction` обязательна и не пробельная (иначе exit 2 до обращения к store), `--comment` (повторяемый) — id замечаний, которые поручение несёт с собой. Та же операция, что `POST /changes/{id}/decisions/{adr}/alternative`: `issue_rework_order` фазы `architecture` с `decision_ids=[<adr>]`, в той же транзакции — version-bound `rejected` по гейту фазы; audit `rework_order.issue`. `--decision` не среди ADR ветки — exit 2; уже открытое (`pending`/`in_progress`) поручение фазы — exit 2 с текстом отказа (`ConversationConflictError`), ничего не записано. Текст: `rework order <id>: <status> (phase=architecture, decisions=<adr>, comments=<n>)` и блок «Следующий шаг»; `--json` — `{"outcome": "created" | "replayed", "rework_order": ReworkOrder, "guidance": Guidance}`.
- **`change ui`** — UI-спека из узлов `design/ui/**` (`orchestration/ui_spec.py: build_ui_spec_view` — тот же view, что `GET /changes/{id}/ui`). Текст (`render_ui_text`): заголовок `ui of <id>: revision=<ревизия фазы interface> dev_url=<…>`; `scenarios (<n>):` — `- SCN-…: <title> [<экраны>]` и шаги `S<n>: <text> -> SCR-…`; `screens (<n>):` — `- SCR-…: <title> route=… preview=…`, `states: <объявленные состояния>`, элементы `EL-…: <kind> <label> (<component>)`; `links (<n>):` — `- SCR-A->SCR-B: <trigger> / <condition>`; ошибки разбора — `! …`. `--section scenarios|screens|links` оставляет одну секцию; в `--json` при этом остаются ключи `change_id`, `revision`, `dev_url`, `errors` и выбранная секция, без `--section` — документ `UiSpecView` целиком. Блока «Следующий шаг» у `change ui` нет.

## 4. Парсинг и обработка ошибок (`main.py`)

- **Дерево команд**: `build_parser()` строит `argparse` с `prog="factory"` и обязательными subparsers (`metavar="command"`) на каждом уровне (включая `product add|validate|list|show|bootstrap`, T070/M2, и `change create|status|phases|answer|comment|rework|approve|artifacts|decisions|alternative|ui`, T073/T086/M3): `stage {run,resume}`, `run {advance,status,publish,withdraw}`, `reconcile`, `outbox {dispatch,replay,skip}`, `doctor`, `api {serve}`, `release {verify}`. Голое `factory`, `factory stage` или `factory outbox` → ошибка argparse, exit 2.
...
- **Общие хелперы (`cli/_common.py`, T119)**: один источник для отчётов об ошибках и доступа к state store у всех команд. `report_error(command, tag, message, code, json_output=…)` печатает `{"error": tag, "detail": message}` в stdout в `--json` и `factory <command>: <message>` в stderr иначе; над ним `report_invalid_input` (exit 2), `report_execution_error` (exit 1) и `report_state_store_unreachable` (фиксированный текст, exit 2). `open_state_store()` — контекст-менеджер «engine → пробное соединение → session factory → dispose»; недоступный или неверно настроенный store поднимает `StateStoreUnreachableError`, и команда отвечает exit 2, не печатая URL (ADR-009). Там же `database_url()`, `default_owner_id(env_var, prefix)` и `os_error_reason(exc)`. Модульные `render_text`/`render_json` остаются в командах — они специфичны для типа отчёта.
- **Заглушки**: `_not_implemented(command, planned_task)` печатает диагностику (текст — stderr, JSON — stdout) и возвращает exit 2.
- **Циклические импорты**: `cli._common`, `cli.stage`, `cli.reconcile`, `cli.outbox`, `cli.api`, `cli.runs` импортируют args-датаклассы и коды выхода из `main`, поэтому `main` импортирует их лениво — внутри обработчиков.
- **`main(argv, *, executor=None, revision_of=None, gate_facts=None, ci_toggles=None, ci_repository=None, provisioning=None, brief_formulator=None, repository=None)`** возвращает `int` — код процесса. Необязательные keyword-only швы (типы под `TYPE_CHECKING`) — точка связывания composition root'а: `executor`/`revision_of`/`gate_facts` потребляет только ветка `RunAdvanceArgs`; `provisioning` — `api serve`, `product validate` и `product bootstrap`; `repository` (T086, M3) — `api serve`, команды `change status|phases|answer|comment|rework|approve|artifacts|decisions|alternative|ui`, которым нужны ревизии артефактов, и `run advance`: драйвер вычисляет по решениям и ревизиям фаз фазу попытки стадии `specification` (`state/phases.py: current_change_phase`, ADR-039) и передаёт её в `StageContext.phase`; без них — детерминированный путь и честные отказы (`artifacts`, `decisions`, `alternative`, `ui` — exit 2). `SystemExit` с кодом поднимает обёртка: console-script `factory` (`pyproject [project.scripts]` → `dark_factory.runtime.entrypoint:main`, ADR-025) или `__main__.py` (`raise SystemExit(main())`). `python -m dark_factory.cli` — явный core-путь: без сборки runtime и без швов.
...
| Команда, которой композиция не нужна (`doctor`, `stage run`, `run status`, `change create`, …) | runtime не собирается; швы = `None` (ADR-025) |
| `change artifacts …` без шва `repository` (core-путь `python -m dark_factory.cli`) | exit 2 до обращения к store: документ, который фабрика не может прочитать, не показывается (ADR-035 p.1) |
| `change decisions` / `alternative` / `ui` без шва `repository` | exit 2 до обращения к store — то же правило, что у `artifacts` (ADR-035 p.1) |
| `change alternative`: `--decision` не среди ADR ветки / `--instruction` пробельная / у фазы `architecture` уже есть открытое поручение | exit 2 `invalid_input` с причиной; ничего не записано |
| `change approve` при закрытом гейте фазы / `change rework` при уже ожидающем поручении | exit 2 с причинами гейта / с id существующего поручения; ничего не записано |
| `change artifacts edit`: файл менялся после `--base-revision` | exit 1 `execution_error` (конфликт не разрешается молча, ADR-035 p.3); та же содержимое, что на голове — no-op, exit 0 |
...
| `change create`: `--limit-usd` не число / ≤ 0 / > 4 знаков, `--brief-json` нечитаем или не по схеме, `--brief-json` вместе с флагами брифа | exit 2 `invalid_input` до обращения к store |
| `change create`: неизвестный `--product` | exit 2 `invalid_input`, ничего не записано |
| `change create`: `--id` уже есть | replay: exit 0, сохранённое не перезаписывается, audit `replayed`, тот же блок «Следующий шаг» |
| `change create`: бриф без problem/goal | задача создана как `draft`, «Следующий шаг» — `Дополнить бриф (api: PUT /changes/{id}/brief)`, запуск требований показан недоступным с причиной |
| `change status`: неизвестная задача | exit 2 `invalid_input` |
...
- [test_cli_changes.py](../../tests/test_cli_changes.py) — `change create|status` над стабами реестра: бриф из флагов и JSON, черновик, сценарий и лимит, replay, парсинг лимита, неизвестный продукт/задача, один и тот же блок «Следующий шаг» после `create` и в `status`, non-echo URL store; интеграция — [tests/integration/test_cli_changes.py](../../tests/integration/test_cli_changes.py) (JSON `guidance` CLI равен проекции из store);
- [test_cli_parser.py](../../tests/test_cli_parser.py) — дополнительно парсинг `release verify`, `run publish`, `run withdraw`, `product *` и `change *`, их флагов и отсутствующих обязательных опций;
- [test_cli_parser_decisions.py](../../tests/test_cli_parser_decisions.py) — парсинг `change decisions|alternative|ui` (обязательные `--id`/`--decision`/`--instruction`, повторяемый `--comment`, `--section` из `choices`) и отказы argparse (exit 2); [test_cli_changes_phases.py](../../tests/test_cli_changes_phases.py) — парсинг `change phases` и `render_phases_text` (текущая фаза помечена, процента нет); интеграция — [tests/integration/test_cli_changes_decisions.py](../../tests/integration/test_cli_changes_decisions.py) (`decisions`/`alternative`/`ui` над PostgreSQL и `FakeRepository`; неизвестная задача и отсутствие шва `repository` — exit 2).
