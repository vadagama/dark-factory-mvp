...
- **`change approve`** — согласование фазы на текущей ревизии через прекондиции гейта (T087): закрытый гейт (блокирующие вопросы, ожидающая/идущая доработка, нет ревизии) — exit 2 с причинами; `--waive` с `--comment` — пропуск фазы с основанием (ADR-032 p.5), без основания — exit 2; `--revision` нужна без шва `repository`. `--json` — `{"outcome", "decision", "phase_gate", "guidance"}`.
...
- **Дерево команд**: `build_parser()` строит `argparse` с `prog="factory"` и обязательными subparsers (`metavar="command"`) на каждом уровне (включая `product add|validate|list|show|bootstrap`, T070/M2, и `change create|status|answer|comment|rework|approve|artifacts`, T073/T086): `stage {run,resume}`, `run {advance,status,publish,withdraw}`, `reconcile`, `outbox {dispatch,replay,skip}`, `doctor`, `api {serve}`, `release {verify}`. Голое `factory`, `factory stage` или `factory outbox` → ошибка argparse, exit 2.
...
- **`main(argv, *, executor=None, revision_of=None, gate_facts=None, ci_toggles=None, ci_repository=None, provisioning=None, brief_formulator=None, repository=None)`** возвращает `int` — код процесса. Необязательные keyword-only швы (типы под `TYPE_CHECKING`) — точка связывания composition root'а: `executor`/`revision_of`/`gate_facts` потребляет только ветка `RunAdvanceArgs`; `provisioning` — `api serve`, `product validate` и `product bootstrap`; `repository` (T086) — `api serve` и команды `change status|answer|comment|rework|approve|artifacts`, которым нужны ревизии артефактов; без них — детерминированный путь и честные отказы (`artifacts` — exit 2). `SystemExit` с кодом поднимает обёртка: console-script `factory` (`pyproject [project.scripts]` → `dark_factory.runtime.entrypoint:main`, ADR-025) или `__main__.py` (`raise SystemExit(main())`). `python -m dark_factory.cli` — явный core-путь: без сборки runtime и без швов.
