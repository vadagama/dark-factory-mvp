...
| GET | `/changes/{change_id}/phase-gate` | Прекондиции гейта фазы (T087): доступность, причины с действием, ревизия (для фаз `specification` — ревизия артефактов самой фазы, ADR-039), состояние согласований (`current`/`stale`/`unbound`, `phase`), `waived`, счётчики, доработка; для `interface` — `ui_requirement` и `checks` (T097) | `PhaseGate`; 404 | открыто |
| GET | `/changes/{change_id}/phases` | Проекция восьми операторских фаз и текущей (T098, ADR-039): состояние, ревизия фазы, ревизия действующего согласования, счётчики вопросов/замечаний, итерация — то, что рендерит левая колонка Console и `factory change phases` | `PhasesProjection`; 404 | открыто |
...
| GET | `/changes/{change_id}/decisions` | Карточки архитектурных решений (ADR) изменения с производным статусом `proposed` / `accepted` / `needs_revision` / `superseded`, открытым поручением «Запросить альтернативу» и затронутыми артефактами (T093, ADR-039 p.8) | `DecisionsView`; 404, 503 без репозитория | открыто |
| POST | `/changes/{change_id}/decisions/{decision_id}/alternative` | «Запросить альтернативу»: поручение на доработку фазы `architecture` по одному решению (`decision_ids=[decision_id]`) через общий `issue_rework_order`; в той же транзакции — version-bound `rejected` по гейту фазы (T093, ADR-039 p.9) | `ReworkOrder`; 201, 200 replay по `Idempotency-Key`, 404 неизвестное решение, 409 открытое поручение фазы уже есть, 422 пустая инструкция, 503 | Bearer + `changes:write` + роль `operator` |
| GET | `/changes/{change_id}/ui` | UI-спека изменения: сценарии, экраны, связи и компоненты из `design/ui/**` на голове ветки плюс `dev_url` продукта (T094, ADR-039 p.10) | `UiSpecView`; 404, 503 | открыто |
...
Решения и UI-спека (T092–T098, ADR-039, M3). Стадия `specification` исполняется раундами по фазам `requirements → architecture → interface` (роль и скилл раунда — по фазе: `product`/`spec-authoring`, `architect`/`solution-design`, `design`/`ui-spec`); текущая фаза вычисляется как первая без действующего решения, а ревизия фазы — последний коммит, тронувший её артефакты (`spec/**` — требования, `design/**` без `design/ui` — архитектура, `design/ui/**` — интерфейс), поэтому раунд архитектора не делает согласование требований неактуальным. `Decision.phase` различает решения на общем гейте `specification`. Карточка решения (`orchestration/decisions.py` над `context/decisions.py`) — read-model над ADR-файлом в git: frontmatter `id`/`status`/`impact` и секции `## Решение / Обоснование / Альтернативы / Последствия` (английские заголовки тоже читаются); статус карточки **производный** и в файл не пишется: `superseded` — из frontmatter; `needs_revision` — открытое (`pending`/`in_progress`) поручение с этим решением в `decision_ids` или устаревшее согласование архитектуры при ADR, изменённом после него; `accepted` — фаза `architecture` согласована на текущей ревизии и открытого поручения нет; иначе `proposed`. `pending_alternative` — открытое поручение «Запросить альтернативу»; `affected_artifacts` — артефакты `design`/`adr`/`ui`/`spec`, изменённые относительно ревизий, зафиксированных последним выполненным (`done`) поручением по решению. UI-спека (`orchestration/ui_spec.py` над `context/ui_spec.py`) читает узлы `design/ui/scenarios/SCN-*.md` и `design/ui/screens/SCR-*.md` (markdown + YAML frontmatter на узел): сценарии с шагами `S<n>` и ссылками на экраны; экраны с `route`, пятью состояниями `loading / empty / error / success / access` (необъявленное состояние отсутствует в `states`, а не подменяется зелёным), элементами `EL-*` (якоря замечаний) и компонентами UIKit; `links` выводятся из `transitions` экранов (id `from->to`, при повторе пары — `#n`), `components` агрегируют `element.component` по экранам. `preview_url`: абсолютный URL — как есть, относительный — к `dev_url` из `.factory/product/factory.yaml` ветки изменения, без `dev_url` — как написан (хост не выдумывается). Неразобранные узлы и отсутствующие документы попадают в `errors[]`, исключений нет. Гейт фазы `interface` (T097) несёт `ui_requirement{required, source: route / agent / operator / default, reason}` — предложение архитектора из frontmatter `design/overview.md` (`ui: not_required` + `ui_reason`), которое оператор подтверждает решением `waived` (допустимо без `subject_revision`), — и `checks[]` (`axe`, `visual_regression`) со статусом `planned` («запланировано на исполнении», не зелёный) или `not_required`. Без репозитория `GET …/decisions` и `GET …/ui` отвечают 503: карточка, которую фабрика не может прочитать, не показывается.
...
| `ApprovalRequest` (тело) | `POST .../approvals` | `gate` (7 значений Gate), `outcome` (approved/rejected/waived), `subject_revision` (min_length=1) — обязателен для `approved`/`rejected`, для `waived` необязателен (пропуск относится к фазе, а не к документу — ADR-039 p.6), опц. `comment`, `expected_state_revision`, `phase` (M3: по умолчанию — фаза гейта; несогласие с `gate` — 422) |
...
| `DecisionAlternativeRequest` (тело) | `POST …/decisions/{decision_id}/alternative` | `instruction` (min_length=1, не пробельная — иначе 422), `comment_ids[]` (default `[]`); `{decision_id}` — `id` из frontmatter ADR или имя файла без расширения |
...
- [`tests/integration/test_api_decisions.py`](../../tests/integration/test_api_decisions.py) — M3 против PostgreSQL и `FakeRepository` с шаблонами пака на ветке изменения: карточки решений читаются из ADR-файлов, «Запросить альтернативу» создаёт поручение фазы `architecture` с `decision_ids`, UI-спека собирается из узлов сценариев и экранов, 503 без репозитория; [`tests/integration/test_api_conversations.py::test_phases_projection_and_phase_bound_approvals`](../../tests/integration/test_api_conversations.py) — проекция фаз и решения, привязанные к фазе и к ревизии её артефактов;
- [`tests/test_orchestration_decisions.py`](../../tests/test_orchestration_decisions.py) — производный статус карточек (пустой view без репозитория, `proposed` до решений, `accepted` при действующем согласовании, `needs_revision` при открытом поручении и при правке ADR после согласования, `superseded`, `affected_artifacts` после выполненного поручения) и `UiSpecView` с `dev_url`; [`tests/test_context_decisions.py`](../../tests/test_context_decisions.py) — парсер ADR-карточки (шаблон пака, английские заголовки и подразделы альтернатив, fallback на имя файла, битый frontmatter — ошибка в карточке, не исключение); [`tests/test_context_ui_spec.py`](../../tests/test_context_ui_spec.py) — парсер UI-узлов (шаблоны пака, правила `preview_url`, отсутствующие состояния, суффиксы связей, тип узла по имени файла); [`tests/test_orchestration_phases.py`](../../tests/test_orchestration_phases.py) — текущая фаза стадии `specification`, решение до M3 как фаза своего гейта, проекция восьми фаз (stale, `not_required` на `quick`, пропуск UI, intake до первого run); [`tests/test_orchestration_phase_gate.py`](../../tests/test_orchestration_phase_gate.py) — M3-случаи гейта `interface`: не требуется на `quick`, `checks` со статусом `planned` никогда не зелёные.
