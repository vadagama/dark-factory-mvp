...
доказательств (ADR-015 п.4, ADR-006 п.2). Изолированные worktree, исполнение
команд и сбор evidence (`ExecutionPort`, T-012) появятся в этом же слое как
провайдеры.

Публикатор отвечает на один вопрос: **как запись о завершённом run попадает в
Git так, чтобы её можно было найти, воспроизвести и нельзя было незаметно
переписать.** Операционный журнал при этом остаётся в PostgreSQL (ADR-004), а
тяжёлая evidence — в CI artifacts через `ArtifactStorePort`: в Git едет только
индекс.
