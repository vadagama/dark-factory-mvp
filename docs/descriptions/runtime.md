...
| `DARK_FACTORY_WORKSPACE_MIRROR_ROOT` (провижининг, T066–T069) | Порт провижининга отсутствует; заданный корень даёт `ProviderClone` при полных `DARK_FACTORY_GITHUB_*` и `LocalMirror` без них |
...
Провижининг продукта (T066–T069, ADR-031) следует той же дисциплине выбора: полный `DARK_FACTORY_GITHUB_*` вместе с `DARK_FACTORY_WORKSPACE_MIRROR_ROOT` даёт `ProviderClone` — адаптер, который клонирует с провайдера и умеет применять baseline-паки (`bootstrap_baseline`); тот же корень зеркала без App-кредов оставляет `LocalMirror` (операторское зеркало, паки не применяет); без корня порт отсутствует. `DARK_FACTORY_PACKS_ROOT` читается и валидируется **только когда выбран `ProviderClone`**: незаданный корень не мешает клонированию и валидации, а `bootstrap_baseline` без него падает fail-closed; заданный криво (пусто или относительно) — `ValueError` с именем переменной без значения. Явная инъекция `token_provider`/`transport` в `build_runtime` приоритетна (тестовый шов).
...
Точка входа процесса — `dark_factory.runtime.entrypoint:main` (console-script `factory` в `pyproject [project.scripts]`, ADR-025). Модуль — **связывание, а не логика** (ADR-024 п.5): он разбирает команду через `cli.main.parse_command` и собирает runtime из окружения процесса (`build_runtime()`) только для команд, которым нужны биндинги: `run advance` (исполнитель стадии, резолвер ревизии и провайдер гейт-фактов, T-092 S3), `api serve` (переключатели этапов CI, T059; порт провижининга, T066) и `product validate` (порт провижининга, T070), после чего вызывает `cli.main.main(argv, …)` с соответствующими биндингами. Швы протащены значениями: `cli.main.main` → `dispatch` → `_advance_run` → `cli.runner.run_advance_command`. Типы `StageExecutor`/`RevisionResolver` живут в `orchestration.runner` и подключены в CLI под `TYPE_CHECKING`: модуль остаётся core и не тянет драйвер в свой импорт.
...
- `api serve` — собирает runtime ради переключателей этапов CI (T059, ADR-027): биндинги `ci_toggles`/`ci_repository` передаются в CLI аргументом (значения, не импорты), runtime освобождается в `finally`; без `DARK_FACTORY_GITHUB_*` и `DARK_FACTORY_GITHUB_REPOSITORY_SLUG` адаптер не строится, и `/ci/*` честно отвечают «не сконфигурировано». Туда же уходят порт провижининга (`provisioning`, T066 — `cli/api.py` пробрасывает его в `create_app`; до T071 шов терялся, и `POST /products/{id}/validate` через `factory api serve` всегда отвечал 503) и формулировщик брифа `brief_formulator` (T072: `Runtime.brief_formulator()` строит `BriefFormulator` на `harness_of(get_profile(Role.PRODUCT), ())` — роль `product` без инструментов; без `DARK_FACTORY_LLM_*` — `None`, и `POST /briefs/formulate` честно отвечает черновиком);
- `product validate` — собирает runtime ради одного шва `provisioning` (T070, ADR-030 p.4/ADR-031): порт передаётся в CLI значением, runtime освобождается в `finally`; без корня зеркала порт отсутствует (`None`), и команда отказывает с exit 2, не меняя статус продукта (ADR-031 p.6);
- `doctor`, `stage run`, `run status`, `product add`/`list`/`show`, `reconcile`, outbox-команды, `release verify` — зависят только от core: runtime не собирается, окружение сверх нужного самой команде не читается, поведение — ровно как у CLI;
...
- Провижининг: `ProviderClone` выбирается, только когда есть и корень зеркала, и полные `DARK_FACTORY_GITHUB_*`; иначе — `LocalMirror`, без корня — порт отсутствует. `DARK_FACTORY_PACKS_ROOT` не влияет на выбор адаптера: без него клон и `validate` работают, а `bootstrap_baseline` падает fail-closed (ADR-031 p.6).

## 6. Где искать проверки

- [`test_runtime_entrypoint.py`](../../tests/test_runtime_entrypoint.py) — точка входа процесса: `run advance`, `api serve` и `product validate` собирают runtime и передают свои биндинги в CLI, runtime закрывается (в том числе при ошибке команды), команда без связывания runtime не собирает и биндингов не передаёт, ошибка разбора не собирает runtime, `cli.main` доносит швы до `runner.run_advance_command`, а биндинги этапов CI — до `cli.api.run_api_serve_command`, и без швов ведёт себя как раньше;
- [`test_runtime_composition.py`](../../tests/test_runtime_composition.py) — пустая и полная конфигурация, сборка исполнителя и её отсутствие без `ExecutionPort`, привязка инструментов роли к harness, громкий отказ `harness_of`, fail-closed telemetry, отсутствие `facts_provider()` без GitHub-конфига, выбор провижининга (`ProviderClone` при App-кредах, `LocalMirror` без них, `None` без корня зеркала), `aclose`;
...
- [ADR-031](../adr/ADR-031-repository-provisioning.md) — провижининг репозитория продукта: выбор `ProviderClone`/`LocalMirror` и `bootstrap_baseline`.
