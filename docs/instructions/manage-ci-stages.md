...
- **Значение `true`** (ровно так, строчными) — этап пропускается.
- **Любое другое значение** (пусто, `false`, `True`, `yes`, `1`, опечатка) — этап выполняется. Это сделано намеренно (fail-safe): опечатка или забытая переменная не может тихо убрать проверку из пайплайна.
...
| Переменную поставили, а этап всё равно выполнился | Значение не ровно `true` (регистр, пробелы) или прогон стартовал до изменения | `gh variable list`; перезапустить прогон |
