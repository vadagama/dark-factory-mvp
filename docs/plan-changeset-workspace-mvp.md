...
- Статус: принят оператором 2026-09-19; работы — `specs/001-dark-factory-mvp/tasks.md` T061–T118 (декомпозиция вех M0–M5).
...
| W | Содержание | Задачи (вехи) | ADR |
|---|---|---|---|
| **W1** Продукт и репозиторий | `Product`, статусы валидации, провижининг (validate/clone/bootstrap), миграции | M1 (T065–T069) | 030, 031 |
| **W2** Обсуждения и правки | `Question`/`Answer`, `Comment` с якорем, `ReworkOrder`, лимит 3, «просмотрено» ≠ «согласовано» | M2 (T078–T081) | 034 |
| **W3** Документы-артефакты | read-model дерева, ревизии/diff, frontmatter-свойства, write-through в git, черновики, staleness | M2 (T082–T086) | 035 |
| **W4** Движок: фазы, гейты, guidance, merge, управление | `Phase`-проекция, прекондиции гейтов, `Guidance`, помощник merge, pause/resume/cancel/correct, hard-stop бюджета | M1–M5 (T074, T087, T098, T103, T112–T113) | 032, 033, 036 |
| **W5** API | `/products`, ChangeSet-ресурсы, `/attention`, `/activity`, merge-assist, обновление `docs/descriptions/api.md` | M1–M5 (T066, T074, T086, T098, T102, T108, T111–T113) | 033 |
| **W6** CLI | `product add/validate`, `change create/status/answer/approve/rework/run/artifacts/merge`, интерактивный Q&A | M1–M5 (T070, T073, T086, T117) | 033 |
| **W7** Console | IA (Продукты · Требует внимания · Активность), экран ChangeSet, компонент «Следующий шаг», intake, требования, архитектура, UI, план, исполнение, демонстрация, доставка, markdown-редактор | M1–M5 (T075–T076, T088–T090, T095–T096, T105–T106, T114–T116) | 037 |
| **W8** Доставка и проверка | Модель промоушена (TD-031), превью + smoke, снятие блокеров пилота (B1/B2, TD-030), e2e-прогон калькулятора | M0, M5 (T062–T064, T110–T111, T118) | — |
| **W9** Документы | ADR-030…037, HLD, техдолг, трекер | T061 | — |

Полный перечень задач каждой вехи — в трекере (`specs/001-dark-factory-mvp/tasks.md`, Phase 11); детализация внутри каждой задачи — при её старте (Native SDD Core, ADR-020).
...
```mermaid
flowchart TD
    M0["M0. Предусловия: снять блокеры пилота (T062–T064)"]
    M1["M1. Продукт, репозиторий, intake (T065–T077)"]
    M2["M2. Обсуждения, документы, требования (T078–T091)"]
    M3["M3. Архитектура и интерфейс (T092–T099)"]
    M4["M4. План и исполнение (T100–T107)"]
    M5["M5. Демонстрация, доставка, закрытие (T108–T118)"]
    M0 --> M1 --> M2 --> M3 --> M4 --> M5
```

| Milestone | Что оператор может | UX-критерий | Задача |
|---|---|---|---|
| **M0** | — (гейты пилота перестают клинить) | — | T062–T064 |
| **M1** | Создать продукт, увидеть «репо валиден», создать задачу с выбором сценария | На каждом экране один CTA и «что дальше» | T065–T077 |
| **M2** | Пройти Q&A, править md, согласовать или отправить на доработку | Вопрос отвечается в один клик; после действия — сводка | T078–T091 |
| **M3** | Принять/отклонить архитектуру и UI | «Запросить альтернативу» без ухода с экрана | T092–T099 |
| **M4** | Утвердить план и запустить, наблюдать, вмешаться | Видно, что и почему мешает; лимит честный | T100–T107 |
| **M5** | Получить dev-превью, дать замечание, rework, принять, смёржить из консоли, закрыть | Merge-помощник объясняет каждый блокер; baseline подтверждается явно | T108–T118 |

Порядок: **M0** (снятие блокеров пилота) — предусловие; затем **M1 первым** (решение оператора) — он снимает главный блокер «не могу завести продукт и задачу из интерфейса». Полный перечень задач каждой вехи — в `specs/001-dark-factory-mvp/tasks.md` (Phase 11).
...
| Задачи | `specs/001-dark-factory-mvp/tasks.md` Phase 11 — T061–T118 (M0–M5) |
