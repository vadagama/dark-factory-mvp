...
- **ADR-028** (`docs/adr/ADR-028-human-gates-at-design-phase.md`, принято): `ui` переносится с
...
- **Живая матрица после ADR-028** (`standard`): specification — `specification`+`ui`, оба
...
- **Открытые темы после ADR-028 (не блокируют пилот)**: (а) для R3/R4 полностью человеческие
...
- **Живой статус p02**: stays `blocked` на `review_verification` (попытка 1 израсходована до
  ...
  который по ADR-028 должен запарковаться в `waiting` на человеческий merge дизайн/констракт-CR.
