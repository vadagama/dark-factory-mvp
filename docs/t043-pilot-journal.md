...
- **Основание (решение оператора).** Вместо добора набора D3 (7/10) первый проход сводится к
  стабилизации **одной формы изменения** — ручными прогонами, пока сквозной путь не станет
  чистым. Причины: наблюдаемые дефекты сквозные (B1, B2, B6, B7, B8), а не задачные; десять
  параллельных изменений давали шум (конфликтующие CR, merge-forward, лишние planning-прогоны);
  метрика vision §7 сформулирована в **запусках** («≥ 7 из 10 запусков», стоимость «измеряется
  с 10-го запуска»), а не в задачах.
- **Решение D9:** форма — **standard-R2**; режим — ручной (оператор ведёт гейты, merge — человек,
  ADR-011); K = 1 чистый прогон, второй — по отдельной команде оператора. Набор D3 и матрица
  5 quick + 5 standard становятся **следующей фазой**, а не первым проходом.
- **Вход формы:** `p06`/`p10` выпали (B6 ×2), `p07` — B8; форма набирается из живых R2-изменений
  `p08` (Alembic-миграция) или `p09` (Notes: `GET /api/notes` + frontend). Финальный выбор —
  оператор при старте W3. Судьба `p06`/`p07`/`p10` (решения D7/D8 и пере-intake) перестаёт быть
  блокирующей для приёмки — она вне объёма стабилизации.
- **Критерий «чисто» (по run records):** `review_verification` = `succeeded` (парковка на `release`
  ожидаема — TD-031); ноль ручных правок артефактов агента; вмешательств человека не-approvals
  ≤ 1 (vision §7); `blocked` по дефекту конвейера — 0; раунды rework ≤ 1 (жёсткий лимит 3);
  все гейты `passed` привязаны к текущему head SHA.
- **Принятое ограничение (записано явно):** K = 1 не отличает «стабильно» от «повезло»
  (в журнале уже был разовый ReadTimeout LLM) — вердикт пилота будет «пройден на одном чистом
  прогоне, повторяемость не доказана». Непокрытыми остаются quick-сторона SC-002 (lead time
  quick/standard), quick-маршрут и вторая половина матрицы форм.
- **Маршрутизация — вариант A (решение оператора).** Пробел «маршрут рана не выбирается
  политикой» в этом цикле **не чинится**: `select_route` реализован и покрыт тестами, но не
  вызывается из продакшн-кода, единственный call-site `store.create_run`
  (`cli/runner.py::_resolve_run`) передаёт константу `DEFAULT_RUN_ROUTE = Route.STANDARD`, а у
  рантайм-`Change` нет профиля workflow (он только в SDD-манифесте ChangeSet). Для standard-R2
  результат совпадает с политикой (`select_route(product-feature, R2) = standard`). Зафиксирован
  как **TD-032**.
- **Следствие:** переписаны `docs/plan-t043-closure.md` (§1, §3, §5 W3, §6, §7, §8, §10) и
  формулировка T043 в `specs/001-dark-factory-mvp/tasks.md`.
