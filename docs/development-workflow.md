...
Штатный порядок выполнения любой задачи, меняющей файлы репозитория (код, тесты, документация, конфиги). Цикл обрамляет реализацию: роль `devops` подключается в начале (ветка) и в конце (MR), а merge остаётся за человеком. Короткое описание — в `AGENTS.md`, поведение оркестратора — в `.agents/skills/dark-factory/SKILL.md`.

## Шаги

| # | Шаг | Роль | Действие | Результат |
|---|---|---|---|---|
| 1 | Ветка | `devops` | Синхронизировать `main` и создать от него ветку задачи, запушить | Ветка задачи в origin |
| 2 | Реализация | `software-engineer` | Внести минимальные изменения в ветке задачи | Коммиты в ветке |
| 3 | Проверка | `software-engineer` | Фактически запустить диагностику, тесты, lint/typecheck по изменённым файлам | Зелёные проверки + evidence |
| 4 | MR | `devops` | Закоммитить, запушить и открыть MR против `main` с описанием и evidence | Открытый MR |
