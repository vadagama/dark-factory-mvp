...
- **Дата**: 2026-09-16
- **Автор**: architect
- Закрывает отложенные условия [ADR-018](ADR-018-human-participation-autonomous-execution.md) п.5 («риск повышен до R2+» — ручная оценка до T-080) и [ADR-011](ADR-011-risk-based-merge-release-policy.md) п.5 («вход политики — риск-классы R0–R4 (T-080)»); реализует историческую задачу T-080 (исторический реестр задач, удалён 2026-09-17), чеклист `specs/001-dark-factory-mvp/tasks.md` T045.
...
- **Требование DoD T-080.** «Маршрутизация детерминирована; опасные изменения не проходят по короткому пути; условие эскалации R2+ работает как гейт, а не как ручная оценка» (T-080 — исторический номер задачи из исторического реестра задач, удалён 2026-09-17).
