...
- Q-13 plan §5: HLD оставил выбор UI pack пилота открытым (`hld-mvp.md` §12/§16); за Small UIKit на React — `strategy.md`, `research/figma-vs-code-html-css-js.md`, `dark-factory-console.md`, `ui-ux.md`, `uikit-radix-shadcn.md` (фиксирует «React поверх Radix/shadcn, не Web Components»), `self-improvement.md`.
...
4. Console фабрики — тоже React + Radix/shadcn (T-051): одна базовая UI-технология фабрики и продуктов; при этом Console **не определяет** UI Kit создаваемых продуктов (`hld-mvp.md` §2).
...
| Web Components UIKit | Фреймворк-агностичность | Отвергнут `uikit-radix-shadcn.md`; агентный код проще на React | Отклонено |
| Figma-центричный процесс (спецификация в Figma) | Привычный дизайн-инструмент | Спека не исполняема и не гейтится в CI; дороже для агентного цикла | Отклонено (`research/figma-vs-code-html-css-js.md`) |
