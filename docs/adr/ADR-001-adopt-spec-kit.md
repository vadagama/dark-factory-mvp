...
- Слой Spec-Driven Development признан необходимым (снапшот `docs/notion/architecture/spec-driven-development.md`, не каноническая документация): стабильный контракт «человек ↔ агенты», контроль решений до генерации кода, проверяемые quality gates, координация нескольких агентов.
- То же исследование рекомендовало OpenSpec как основной формат изменений, а из Spec Kit — перенести только quality gates (clarify, analyze, checklist, converge). OpenSpec в репозитории не установлен и не настраивался.
...
6. Рекомендация Notion-снапшота «OpenSpec как основной SDD» на bootstrap-фазе перекрывалась настоящим ADR; целевым решением принят единый OpenSpec ([ADR-017](ADR-017-unified-openspec-sdd-factory-profile.md)). Сам снапшот остаётся без изменений как история исследований.

## Альтернативы

| Вариант | Плюсы | Минусы | Почему не выбран |
|---|---|---|---|
| OpenSpec как основной SDD (рекомендация исследования) | Заточен под непрерывные brownfield-изменения, лёгкий change-процесс | Не установлен; brownfield-сценарий пока отсутствует (pre-MVP, greenfield) | Отложено; переоценено — [ADR-017](ADR-017-unified-openspec-sdd-factory-profile.md) принял единый OpenSpec целевым форматом |
...
**Негативные / риски**
- Осознанное расхождение с рекомендацией Notion-исследования (OpenSpec-first) — зафиксировано здесь.
