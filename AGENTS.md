...
- Статус: pre-MVP, MVP-ядро реализовано (US1–US6: CLI-стадии, SDD, гейты/merge-политика, GitOps-релиз, API/Console; durable-раннер — ADR-024/025); впереди e2e-пилот (T043) и финальная приёмка (T056). Архитектурные решения приняты (ADR-001…ADR-027).
...
- Секреты и ключи — никогда в коде и в git. Локальные секреты лежат в `.env` в корне репозитория (не коммитится; локальное зеркало секретов Kubernetes namespace `factory`, источник истины — секреты кластера: `kubectl -n factory get secret <имя>`). Агенту запрещено читать `.env`, выводить или копировать его содержимое в код, доки, логи, коммиты и промпты. Процессам секреты передаются только через окружение (`--env-file .env`); имена ключей — в `.env.example` (без значений).
...
.
├── AGENTS.md                  # этот файл
├── .env                       # локальные секреты (НЕ в git, агентам не читать): зеркало секретов k8s namespace factory; шаблон имён ключей — .env.example
