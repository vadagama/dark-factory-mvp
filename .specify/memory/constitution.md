...
Основание: запрос пользователя (2026-09-13) — каждая задача выполняется в отдельной
  ветке, проверяется и завершается MR роли `devops`.
Modified principles:
...
(env, хранилища). Git-цикл задачи — штатная часть конвейера: `devops` создаёт ветку
...
- Конвейер: задача → `dark-factory` (классификация, план) → роли (`analyst`,
  ...
  → приёмка по DoD.
- Git-цикл задачи (обязательный): `devops` создаёт ветку до реализации,
  `software-engineer` реализует и проверяет, `devops` открывает MR после зелёной
