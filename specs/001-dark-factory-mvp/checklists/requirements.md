...
- Имена внешних систем (GitLab, Console, GitOps/Argo CD, трекер) сохранены в спецификации осознанно: это требования интеграции продукта (фабрики) из HLD, а не внутренние решения реализации. SDD-слой — собственный Native SDD Core (ADR-020), не внешняя система. Внутренние технологические выборы (язык ядра, агентный фреймворк, библиотека графов, стек Console) в спецификацию не включены и определяются в `/speckit-plan`.
