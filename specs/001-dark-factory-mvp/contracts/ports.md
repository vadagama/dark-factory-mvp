...
class RepositoryPort(Protocol):
    ...
    async def list_tree(self, repository: RepositoryRef, ref: str, /, *, prefix: str = "") -> Sequence[str]: ...
    async def list_commits(self, repository: RepositoryRef, ref: str, /, *, path: str | None = None) -> Sequence[CommitInfo]: ...
...
`read_file` / `list_tree` / `list_commits` (T082, ADR-035 — аддитивное расширение по образцу `observe`, без ADR) — read-модель документов-артефактов: git — источник истины, поэтому фабрика читает документ из репозитория на ревизии, а не хранит копию. Только чтение, без `idempotency_key` (чтение не минтует внешнего эффекта, FR-017). `read_file(repository, ref, path)` — байты файла на ветке или SHA; `list_tree(repository, ref, prefix=)` — отсортированные пути блобов под префиксом; `list_commits(repository, ref, path=)` — коммиты ветки, затронувшие путь (все — при `path=None`), новые первыми, как `CommitInfo(sha, message, author?, authored_at?)`. Отсутствующий ref или путь — `KeyError` («404 = отсутствует»); отсутствующий префикс или путь без коммитов — пустая последовательность. GitHub-адаптер — contents API, recursive trees API дерева головы и commits API с фильтром `path`; фейк резолвит имя ветки или известный SHA. Потребитель — `orchestration/artifacts.py` (`ArtifactService`), сервис API/CLI артефактов.
...
Подготовка репозитория продукта до того, как его коснётся стадия агента (T067–T069, ADR-031 p.1). Два адаптера за одним портом (p.2): `LocalMirror` — операторское локальное зеркало (T067), `ProviderClone` — клон с провайдера на installation-токене GitHub App, без PAT (T068). `validate` — read-only проба без ключа и без мутаций (p.5): `UNAVAILABLE` (зеркало или доступ недоступны), `EMPTY` (unborn HEAD — штатный случай, p.4: baseline его и создаст), `BASELINE_ABSENT`/`BASELINE_CURRENT` (`BASELINE_STALE` зарезервирован и не выставляется — TD-038); недоступность репозитория — evidence (p.6), а не исключение. `ensure_mirror` — replay-дедуп по ключу, отсутствующий репозиторий — `KeyError` (404 = отсутствует). `bootstrap_baseline` реализован в `ProviderClone` (T069), а адаптер без этой возможности (`LocalMirror`) бросает `ProvisioningOperationUnsupportedError` с именем операции — молчаливый success запрещён (p.6). Контракт применения: паки — данные из необязательного корня `DARK_FACTORY_PACKS_ROOT` (без него клон/валидация работают, bootstrap падает fail-closed); манифест пака (`pack.yaml`, `dark-factory.dev/pack/v1`, непустой `name`, SemVer `version`) даёт `AppliedPack`, поддерево `baseline/` применяется к корню репозитория как `.factory/`; рабочее зеркало выравнивается на текущую вершину ветки по умолчанию провайдера, payload ставится в индекс принудительно (`git add --all --force`), коммит уходит как `HEAD:refs/heads/<default>` installation-токеном, который попадает в git только через окружение дочернего процесса (инвариант T068 — не argv и не URL), а фактическая вершина ветки сверяется у провайдера (`git ls-remote`) перед выдачей evidence; пустой репозиторий (unborn HEAD) — штатный случай, создающий первый коммит; идемпотентность — replay по `idempotency_key` и физический no-op, когда payload не поставил изменений при уже присутствующем `.factory/product` в HEAD (второго коммита нет, evidence — паки из subject коммита baseline; baseline, созданный не bootstrap'ом, — пусто), а при отсутствии изменений без baseline — fail-closed; evidence — `BaselineBootstrapResult(revision, applied_packs)` (p.6). Реализация T067 — `LocalMirror` + `FakeRepositoryProvisioning`. Реализация T068 — `ProviderClone` (`adapters/scm/github/provisioning.py`): клон/fetch на installation-токене GitHub App (PAT не вводится), токен передаётся git только через окружение дочернего процесса — не в argv и не в URL; `validate` не мутирует репозиторий и не трогает локальное зеркало, пустой репозиторий сохраняет `default_branch`.
