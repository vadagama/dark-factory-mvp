...
Этап — job `.github/workflows/ci.yml`; переключатель — `CI_SKIP_<JOB>` в repository variables GitHub (ADR-026). Каталог этапов — код (`dark_factory.ci.stages`), он же выводит имя переменной, поэтому запрос не может назвать произвольную переменную репозитория (T059, ADR-027).
